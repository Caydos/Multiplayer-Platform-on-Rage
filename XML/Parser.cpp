...
void Parser::AddTag(Tag _tag, unsigned int& _tagCount, Tag*& _tags)
{
	if (_tagCount)
	{
...
void Parser::SearchForTags(const char* _buffer, unsigned int& _tagCount, Tag*& _tags)
{
...
void Parser::GetAttributesFromBuffer(const char* _buffer, Tag& _tag)
{
	char* searchBuffer = (char*)_buffer;
	bool searching = true;
	while (searching)
...
XML::File Parser::ReadFile(const char* _fileName)
{
	File file;
...
char* Parser::GetFileAsString(const char* _fileName)
{
...
XML::Tag* Parser::GetTag(const char* _name, unsigned int _tagCount, Tag*& _tags)
{
...
XML::Attribute* Parser::GetAttribute(const char* _name, unsigned int _attributesCount, Attribute*& _attributes)
{
...
const char* Parser::GetAttributeContent(const char* _name, unsigned int _attributesCount, Attribute*& _attributes)
{
	for (unsigned int i = 0; i < _attributesCount; i++)
	{
		if (strcmp(_attributes[i].name, _name) == 0)
